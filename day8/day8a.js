...
var testinput = `b inc 5 if a > 1
a inc 1 if b < 5
c dec -10 if a >= 1
c inc -20 if c == 10`;
var input = `aj dec -520 if icd < 9
z dec -500 if b <= 2
zz dec 628 if z >= 499
db dec -818 if u >= 0
...
for (var i = 0; i < instructionArray.length; i++) {
    var instruction = instructionArray[i].split(" ");
    var target = instruction[0];
    var operator = instruction[1];
    var amount = parseInt(instruction[2]);
    var tester = instruction[4];
    var test = instruction[5];
    var testValue = instruction[6];
    if (registers[target] == undefined) {
        registers[target] = 0;
    }
    if (registers[tester] == undefined) {
        registers[tester] = 0;
    }
    if (eval(registers[tester] + " " + test + " " + testValue)) {
        if (operator == "inc") {
...
var arr = Object.keys( registers ).map(function ( key ) { return registers[key]; });
