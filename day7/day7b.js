...
var discArray = input.split("\n");
var tree = {};
var lines = discArray.length;
for (var i = 0; i < lines; i++) {
    var parts = discArray[i].split(")");
    var name = parts[0].split(" (")[0];;
    var weight = parseInt(parts[0].split(" (")[1].replace(")",""));
    var above = parts[1].split(" -> ")[1];
    if (above != null) {
        aboveArray = above.split(", ");
        above = {};
        for (j = 0; j < aboveArray.length; j++) {
            above[aboveArray[j]] = 0;
        }
        tree[name] = {weight: weight, supportingWeight: null, above: above, weighed: 0};
    } else {
        tree[name] = {weight: weight, supportingWeight: 0, above: {}, weighed: 0};   
    }
}
//var base = tree["tknk"]; // test
var base = tree["gynfwly"]; // real
while (base.weighed < Object.keys(base.above).length) {
    for (var key in tree) {
        var disc = tree[key];
        var aboveLength = Object.keys(disc.above).length;
        if (disc.weighed < aboveLength) {
            for (k2 in disc.above) {
                var info = tree[k2];
                if (Object.keys(info.above).length == info.weighed && disc.above[k2] == 0) {
                    if (Object.keys(info.above).length > 0) {
                        var supportingWeight = Object.values(info.above).reduce((a, b) => a + b);
                    } else {
                        var supportingWeight = 0;
                    }
                    disc.above[k2] = info.weight + supportingWeight;
                    disc.supportingWeight = disc.supportingWeight + disc.above[k2];
                    disc.weighed = disc.weighed + 1;
                }
            }
        }
    }
...
for (key in tree) {
    if (Object.keys(tree[key].above).length > 0) {
        var weightArray = Object.keys( tree[key].above ).map(function ( k2 ) { return tree[key].above[k2]; }); 
        if (new Set(weightArray).size != 1) {
            console.log(key + ": " + JSON.stringify(tree[key]));
        }
    }
}
