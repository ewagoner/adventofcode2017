...
for (var i = 0; i < lines; i++) {
    ...
    if (above != null) {
        above = above.split(", ");
        discArray[i] = [name,above];
    } else {
        discArray[i] = [name];        
    }
}
while (discArray.length > 1) {
    for (var i = 0; i < discArray.length; i++) {
        if (discArray[i][1] == undefined) {
            var name = discArray[i][0];
            for (var j = 0; j < discArray.length; j++) {
                if (discArray[j][1] != undefined) {
                    var k = discArray[j][1].indexOf(name);
                    if (k != -1) {
                        discArray[j][1].splice(k, 1);
                    }
                }
                if (discArray[j][1] != undefined && discArray[j][1].length == 0) {
                    discArray[j].splice(1, 1);
                }                
            }
            discArray.splice(i, 1);
        }
        if (discArray[i] != undefined && discArray[i][1] == []) {
            discArray[i].splice(1, 1);
        }
    }
}
